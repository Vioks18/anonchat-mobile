...
interface MessageStore {
  ...
  deleteForMe: (messageIds: string[], userId: string) => void;
  deleteForAll: (messageIds: string[], userId: string) => Promise<void>;
...
if (__DEV__) console.warn('getMessagesByStatus: Невалидный статус');
  ...
  // Методы удаления сообщений
  deleteForMe: (messageIds: string[], userId: string) => {
    try {
      if (!messageIds || !Array.isArray(messageIds) || !userId) {
        if (__DEV__) console.warn('deleteForMe: Невалидные параметры');
        return;
      }
      ...
      set((state) => {
        const updatedMessages = state.messages.map((msg) => {
          if (messageIds.includes(msg.id)) {
            // Добавляем userId в deletedFor, избегая дубликатов
      ...
      if (__DEV__) console.error('deleteForMe: Ошибка удаления для себя', error);
    }
  },

  deleteForAll: async (messageIds: string[], userId: string) => {
    try {
      if (!messageIds || !Array.isArray(messageIds) || !userId) {
        if (__DEV__) console.warn('deleteForAll: Невалидные параметры');
        return;
      }

      const state = get();
      
      // Проверяем что все сообщения принадлежат пользователю
      const messagesToDelete = state.messages.filter(msg => 
        messageIds.includes(msg.id) && msg.sender === "me"
      );

      if (messagesToDelete.length !== messageIds.length) {
        if (__DEV__) console.warn('deleteForAll: Не все сообщения принадлежат пользователю');
        return;
      }

      // Проверяем окно удаления (2 минуты)
      const deleteWindowMs = 120000; // 2 минуты
      const now = Date.now();
      const withinWindow = messagesToDelete.every(msg => 
        (now - msg.timestamp) <= deleteWindowMs
      );

      if (!withinWindow) {
        if (__DEV__) console.warn('deleteForAll: Сообщения вне окна удаления');
        return;
      }
      ...
      try {
        const { deleteForAll } = await import('../providers/FakeServerAPI');
        await deleteForAll(messageIds);
      } catch (error) {
        // Откатываем изменения при ошибке
        if (__DEV__) console.warn('deleteForAll: Ошибка сервера, откатываем изменения');
